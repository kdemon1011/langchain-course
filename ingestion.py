...
tavily_map = TavilyMap(max_depth=5, max_breadth=25, limit=1000)
...
async def index_documents_async(documents: List[Document], batch_size):
...
async def main():
    ...
    # Debug: check what TavilyMap returned
    if "error" in site_map:
        log_error(f"TavilyMap returned an error: {site_map['error']}")
        return
    if "results" not in site_map:
        log_error(
            f"TavilyMap unexpected response. Keys: {list(site_map.keys())}. Response: {str(site_map)[:500]}"
        )
        return
